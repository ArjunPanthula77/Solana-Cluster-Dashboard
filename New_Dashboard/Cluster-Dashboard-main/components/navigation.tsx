...
import { Button } from "@/components/ui/button"
import { ThemeToggle } from "@/components/theme-toggle"
import { Home, Menu, X, LogIn, LogOut, UserPlus, Mail } from "lucide-react"
import { useState, useEffect } from "react"
import { cn } from "@/lib/utils"
...
export function Navigation({ setIsLoggedInState }: { setIsLoggedInState?: (isLoggedIn: boolean) => void }) {
  ...
  const [supportFormMessage, setSupportFormMessage] = useState("")
  ...
  useEffect(() => {
    ...
    if (storedIsLoggedIn === "true") {
      setIsLoggedIn(true)
      if (setIsLoggedInState) setIsLoggedInState(true)
    } else {
      const checkSession = async () => {
        try {
          let res = await fetch("https://solana-cluster-dashboard-production-cce9.up.railway.app/me", {
          ...
          const loggedIn = res.ok
          setIsLoggedIn(loggedIn)
          if (setIsLoggedInState) setIsLoggedInState(loggedIn)
          if (loggedIn) {
            localStorage.setItem("isLoggedIn", "true")
          } else {
            localStorage.removeItem("isLoggedIn")
          ...
          console.error("Session check error:", error)
          setIsLoggedIn(false)
          if (setIsLoggedInState) setIsLoggedInState(false)
          localStorage.removeItem("isLoggedIn")
        }
      }
      checkSession()
  ...
  const handleSupportSubmit = async (e: any) => {
    ...
    if (!supportEmail || !supportMessage) {
      setSupportFormMessage("❌ Please provide both email and message")
      return
    }
    ...
    try {
      const res = await fetch("https://formspree.io/f/support@opsonchain.com", {
        method: "POST",
        body: formData,
        headers: {
      ...
      if (res.ok) {
        setSupportFormMessage("✅ Message sent successfully!")
        setSupportEmail("")
        setSupportMessage("")
        setSupportFiles([])
        setTimeout(() => {
        ...
        setSupportFormMessage("❌ Failed to send message")
      }
    } catch (error) {
      console.error("Support form error:", error)
      setSupportFormMessage("❌ Error sending message")
    }
  }
  ...
  const navItems = [
    { name: "Home", href: "/", icon: Home, current: pathname === "/", onClick: handleHomeClick },
    { name: "Support", href: "#", icon: Mail, current: false, onClick: handleSupportClick },
  ]
    ...
    <>
      ...
      {supportFormOpen && (
        <div className="p-6 max-w-md mx-auto bg-card rounded-xl shadow-md mt-6">
          ...
          {supportFormMessage && <p className="mt-3 text-sm">{supportFormMessage}</p>}
          <Button variant="ghost" size="sm" onClick={() => setSupportFormOpen(false)} className="mt-3 w-full">
            Cancel
          </Button>
        </div>
      )}
